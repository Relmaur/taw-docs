...
MetaBlock supports registering multiple WordPress block variations from a single block class. Pass a `variations` array to the constructor to declare style or layout alternatives that editors can choose in the block editor.
        ...
        parent::__construct([
            'variations' => [
                [
                    'name'        => 'hero-centered',
                    'title'       => 'Centered Hero',
                    'description' => 'Text and image centered',
                    'isDefault'   => true,
                ],
                [
                    'name'  => 'hero-split',
...
Asset IDs (CSS/JS handles) are automatically namespaced per variation so each can enqueue its own styles without collision.
