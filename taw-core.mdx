...
<Columns cols="3">
  ...
  <Card title="Grid view sidebar" icon="layout-panel-left" horizontal={false}>
    A FileBird-style sidebar (Alpine.js) bolted onto the default Media Library Grid view — the same folder tree and full CRUD as the dedicated screen. Clicking a folder filters the grid live via an `ajax_query_attachments_args` filter plus a narrow bridge that sets props on `wp.media`'s existing Backbone query object, not a Backbone *view* override. Stays in sync with the List view's dropdown via the same `taw_media_folder` param.
  </Card>
</Columns>
