...
<Columns cols="2">
  <Card title="Media → Folders" icon="folder-tree" horizontal={false}>
    A dedicated screen: a folder tree (create, rename, delete, drag-and-drop to re-nest) alongside a drag-and-drop attachment grid, including an "Unfiled" pseudo-folder for attachments with no folder assigned. Entirely custom markup and REST calls — no WordPress core Grid-view (Backbone) internals are touched, deliberately, since patching those is fragile across core versions.
  </Card>
