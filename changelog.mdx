...
<Update label="2026-09-03 (pending release)" description="taw-hub-companion 0.1.6: two new signed routes — GET /vulnerabilities (surfaces the site's own scanner findings, no paid feed) and GET /inventory/checksums (per-component file manifest for integrity + backdoor-on-update diffing)" tags={["taw-hub-companion", "feature"]}>
