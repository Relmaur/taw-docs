...
description: "The signed wp-json/taw-hub/v1 receiver plugin that connects a TAW site to the TAW Site Manager control hub."
---

`taw-hub-companion` is a lightweight WordPress plugin that exposes a small, **signature-authenticated** REST surface on a TAW site so a central [TAW Site Manager](https://github.com/Relmaur/taw-site-manager) hub can check its health, run framework syncs, execute allow-listed `bin/taw` commands, and rotate its key — with no passwords.
...
```mermaid
sequenceDiagram
    participant Hub as TAW Site Manager
...
<Steps>
  <Step title="Add the plugin" icon="download">
    Drop `taw-hub-companion/` into `wp-content/plugins/` and run `composer install` inside it (or ship it with `vendor/` bundled).
  </Step>
...
|---|---|---|---|
| `GET` | `/health` | — | `ok`, PHP / WP / `taw/core` / plugin versions, `site_public_key`, `site_key_id` |
...
Every request and response is signed. The full spec is **frozen** and governed by the Hub repo — see [ADR-0003 — Wire protocol & request signatures](https://github.com/Relmaur/taw-site-manager/blob/main/docs/ADR/0003-wire-protocol-and-signatures.md) and [ADR-0005 — Companion plugin architecture](https://github.com/Relmaur/taw-site-manager/blob/main/docs/ADR/0005-companion-plugin-architecture.md).
