...
<ExpandableGroup>
  ...
  <Expandable title="Icons need the Lucide picker switched on" default-open="false">
    `icon` fields need `\TAW\Core\Icons\Lucide::enable()`, called before the theme boots taw/core (in TAW Gutenberg, before `after_setup_theme` priority 0). Without it, the panel says icons aren't switched on. On taw/core v1.51.0, block themes also had to call `Lucide::init()` for the icon search to exist; from **v1.51.1** the data layer does that for you.
  </Expandable>
