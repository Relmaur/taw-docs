...
`bin/taw` scaffolds and introspects the *framework* (blocks, fields, forms) — it knows nothing about actual site *content*. For that, use WordPress's own official CLI, **WP-CLI** (the `wp` command): posts, options, users, terms, transients, arbitrary PHP via `wp eval`/`wp eval-file`, even an interactive `wp shell`. It's a host-level tool, not bundled by this theme, but present on virtually every real WordPress host and every local dev environment (Local, Herd, DDEV).
