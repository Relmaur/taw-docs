...
`TAW\Hub` turns a site into a headless, authenticated client of a central management Hub: telemetry, Vite asset deployment, block-config sync, and cache invalidation over the `wp-json/taw-hub/v1/` namespace. Wired into `Theme::boot()` as the final subsystem.

<Callout kind="warning">
  Opt-in **by server config, not theme code** — there is no `enable()` call. The Hub is a security boundary, so it's switched on only in `wp-config.php`. With `TAW_HUB_ENABLED` unset the integration is completely inert: no routes registered, no hooks added, no options or tables created.
</Callout>
...
define('TAW_HUB_SECRET', '…32+ random bytes…');                // seals the site's own key; falls back to SECURE_AUTH_KEY

// Optional — pin a trusted key directly instead of (or alongside) enrolment:
define('TAW_HUB_KEYS', json_encode([
  'hub-prod' => [
    'secret'       => '…64+ hex…',          // HMAC-SHA256, and/or
...
Every request except `/handshake` carries `X-TAW-Hub-Scheme`, `X-TAW-Hub-Key-Id`, `X-TAW-Hub-Timestamp`, `X-TAW-Hub-Nonce`, and `X-TAW-Hub-Signature`. The signature is HMAC-SHA256 (hex) or Ed25519 (base64) over a frozen `v1` canonical string:

```
v1\n{UPPERCASE METHOD}\n{REST route}\n{sha256-hex of the raw body}\n{unix timestamp}\n{nonce}
```

A ±60 second timestamp drift is enforced, and every nonce is single-use (a WordPress-transient replay cache). The signing key is looked up by `X-TAW-Hub-Key-Id` in `TAW_HUB_KEYS` and/or the enrolled-keys store.

<Callout kind="tip">
  `POST /handshake` trades a valid single-use `TAW_HUB_ENROLMENT_TOKEN` (compared with `hash_equals`) plus the Hub's Ed25519 public key for a stored credential. Granted capabilities are the **intersection** of what the Hub asks for and `hub:read | hub:deploy | hub:config | hub:maintenance` — the `*` wildcard is never grantable by handshake. Enrolled entries hold public keys only, in the `taw_hub_enrolled_keys` option.
</Callout>
...
| ------ | -------- | ---------- | ------- |
| `GET` | `/taw-hub/v1/health` | `hub:read` | environment report — PHP / MySQL / WP versions, ext-sodium, object cache, free disk |
...
| `GET` | `/taw-hub/v1/command/actions` | `hub:read` | the action allow-list and each action's capability |
...
<Callout kind="warning">
  There is **no arbitrary-command endpoint.** `/command` resolves against a fixed registry — `report-telemetry`, `flush-caches`, `sync-blocks`, `deploy-assets`, `rollback-assets` — and re-checks the caller's capability against the *specific* action even after the route guard passes. Asset deploy and rollback are dispatched through `/command` too, so there is exactly one audited execution path.
</Callout>

### Asset deployment safety

A pushed archive is extracted **entry by entry** — never `ZipArchive::extractTo()`. Path traversal (`..`, absolute, backslash, NUL), symlink entries, a non-allow-listed extension (`php` is not on the list), and per-file / total / compression-ratio limits are all rejected before anything is written. The deploy stages into a same-filesystem sibling of the build directory, validates the Vite `manifest.json` against the extracted file set, then swaps by atomic `rename()` — keeping one rollback generation for `rollback-assets`.

Every authorization decision and every dispatched action is written to a `{$wpdb->prefix}taw_hub_audit` table (created on `admin_init` behind a version gate; an in-memory store is the fallback).

### `wp taw hub` (WP-CLI)

Registered only when the integration is enabled and WP-CLI is loaded. Runs through the **same** action registry as `/command` — terminal parity with the Hub over the bounded action set.
