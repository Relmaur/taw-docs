...
- `Importer::plan()` builds the field-level diff (`unchanged` / `changed old→new` / `new` / `would-delete`) and writes nothing. `content:import` without `--yes` stops there; the admin screen renders it as a review table with an explicit "Apply N changes" button.
