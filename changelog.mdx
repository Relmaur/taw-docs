...
<Update label="2026-09-03 (pending release)" description="taw-hub-companion 0.1.5: two new signed routes — GET /vulnerabilities (surfaces the site's own scanner findings, no paid feed) and GET /inventory/checksums (per-component file manifest for integrity + backdoor-on-update diffing)" tags={["taw-hub-companion", "feature"]}>
...
`GET /wp-json/taw-hub/v1/vulnerabilities` — signed, read-only, DB-read-only. A per-scanner read adapter (`src/Security/`) reads the scanner's *stored results* (it never triggers a scan) and normalizes them; `ScannerRegistry` picks whichever scanner the site runs. Ships a **Wordfence** adapter (reads the `wfIssues` table — known vulnerabilities, abandoned plugins, directory removals; reports `last_scan_at` so the Hub can spot a stale scanner; verified against Wordfence 8.2.x). A **Defender Pro** adapter is pending a real Pro install and will take priority once built.
...
Summary mode (no `slug`) returns just `tree_hash` + `file_count` per active component — a cheap "did anything change" poll; detail mode (`?slug=`) adds the full `relpath → sha256` map. Only executable / script file types are hashed; `node_modules` and `.git` are skipped; symlinks are not followed.
