...
<Steps>
  <Step title="Clone and install dependencies" icon="download" title-type="p">
    ```bash
...
Block Bindings (bind core blocks to taw fields in templates), a loop block, and per-post-type
editing policies are on the taw/core roadmap. TAW Gutenberg adopts each one first.
