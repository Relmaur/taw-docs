...
The classic TAW Theme boots TAW Core via the `TAW\Core\Theme::boot()` entrypoint (through `Theme::bootstrapFullSite()`). You can then adjust performance behavior with `Theme::performance([...])`.

A block theme such as [TAW Gutenberg](/taw-gutenberg) boots only the data layer with `\TAW\Core\Boot::data()`. It gets post types, fields, options, REST meta and content interchange, but no blocks, Vite, or performance tweaks. See [Data layer](/data-layer).
...
`screens` takes post types (and page slugs or templates). Since `taw/core` v1.53.0 it also takes `term:<taxonomy>` for a taxonomy's term screens, and since v1.54.0 `user` for the user profile screens. Every field type works on all of them. See [Fields on terms](/data-layer#fields-on-terms) and [Fields on users](/data-layer#fields-on-users).
...
<ExpandableGroup>
  ...
  <Expandable title="Icon" default-open="false">
    Lucide icon picker. Stores a bare icon name (e.g. `house`) sanitized with `sanitize_key()`.

    <Callout kind="tip">
      Opt-in. Call `TAW\Core\Icons\Lucide::enable()` in `inc/customizations.php` before `Theme::boot()`, or the field renders a notice instead of the picker. See [Icon System](#icon-system) below.
    </Callout>
    ...
    <Steps>
      ...
      <Step title="Usage" icon="play" title-type="p">
        ...
        $iconName = Metabox::get($postId, 'feature_icon');
        echo Lucide::render($iconName, ['class' => 'w-6 h-6']);
        ```
      </Step>
    </Steps>
  </Expandable>

  <Expandable title="Gradient Text" default-open="false">
    Ordered list of `{text, highlighted}` segments — for a gradient heading where the highlighted run isn't always the trailing segment (e.g. "How can **we help**?"). The admin UI is a lightweight Alpine-only segment editor (add/remove/toggle); reach for a real `repeater` instead if row reordering matters.
    ...
    <Steps>
      ...
      <Step title="Usage" icon="play" title-type="p">
        ```php
        $segments = Metabox::get_gradient_text($postId, 'heading');
        echo Metabox::renderGradientText(
            $segments,
    ...
    <Callout kind="tip">
      `renderGradientText()`'s highlight class is entirely up to the caller — the actual gradient is a per-theme design decision this framework method has no business hard-coding.
    </Callout>
  </Expandable>

  <Expandable title="HubSpot Form" default-open="false">
    HubSpot embed config — `{portal_id, form_id, region}` — for a block offering a HubSpot embed with a fallback to the site's own [Forms](#forms) system when unconfigured.
    ...
    <Steps>
      ...
      <Step title="Usage" icon="play" title-type="p">
        ...
        $config = Metabox::get($postId, 'hubspot_form');
    ...
    <Callout kind="tip">
      No `enable()` gate — unlike `icon`'s bundled Lucide set, there's no asset here to opt into. `region` defaults to `na1` when blank.
    </Callout>
  </Expandable>

  <Expandable title="link" default-open="false">
    A URL, its text and “open in a new tab”, edited together and stored as one value (`taw/core` v1.59.0+). Relative paths like `/contact` work; with no URL there's no link. `required` means a URL.

    ```php
    ['id' => 'hero_cta', 'label' => 'Button', 'type' => 'link', 'required' => true]
    ```

    ```php
    use TAW\Taw;

    echo Taw::post($postId)->field('hero_cta');               // <a href="…" target="_blank" rel="noopener">Text</a>, escaped
    $cta = Taw::post($postId)->field('hero_cta')->link();     // ->url(), ->label(), ->newTab(), ->exists()
...
<Callout kind="tip">
  Since `taw/core` v1.57.0 there's also a typed API (`Taw::post()`, `Taw::term()`, `Taw::user()`, `Taw::option()`, and `$this->fields($postId)` in blocks), with values escaped when echoed. See [Reading fields](/reading-fields).
</Callout>
...
`class` is appended to the `<form>`'s `taw-form`; `button_class` is appended to `taw-btn taw-btn-primary` on the submit button (and Next, on a multi-step form). Both **add to** the built-in classes rather than replacing them, so a theme overrides only what it needs, scoped to that one form — `.contact-form .taw-input { … }`. Blank or non-string values are ignored.

<Callout kind="info">
  A field's `width` is emitted as the `--taw-span` custom property (1–12 grid columns), which `form.css` turns into `grid-column: span N`. Unlike the inline `grid-column` it replaced, this leaves `grid-column` free to override with an ordinary selector — e.g. `.contact-form .taw-form-field { grid-column: 1 / -1; }` — no `!important`. Every grid cell (fields, `html`, `heading`, `divider`) now collapses to full width below 640px.
</Callout>
...
**Cloudflare Turnstile** is opt-in bot verification. Site/secret keys are PHP constants defined in `wp-config.php` — the same pattern as database credentials, never a metabox or options-page field, because a secret doesn't belong in the database, and an options page can [expose its fields over REST](/data-layer#options-pages-over-rest).
...
1. An admin-configured override for that specific form (Settings → Form Webhook page's **Per-Form Webhooks** table — one row per registered form).
2. A code-level default set via the form's own `webhook` config key:
   ```php
   Form::register([
       'id'      => 'contact',
   ...
   ```
3. The **Default Webhook** at the top of the same settings page — the site-wide fallback for any form with neither of the above.
...
- The unlock cookie is a signed, stateless token (`hash_hmac('sha256', ..., wp_salt('auth'))`, not the plaintext password) — a client can't forge one by just setting a cookie manually.
- Password comparison uses `hash_equals()` (timing-safe).
- Gate attempts are rate-limited per-IP via the same `TAW\Core\Form\RateLimiter` used by `Form`.
- A correct submission redirects (POST/redirect/GET) rather than re-rendering, so a page refresh never resubmits the password.
- Both the gate screen and the unlocked page send `nocache_headers()` — without this, a full-page cache or CDN in front of PHP could serve a cached copy of the *unlocked* page to a different visitor who never entered the password.
...
`TAW\Support\EmailConfig::useEmailit()` routes **all** `wp_mail()` calls — form submissions, password resets, WooCommerce order emails, anything else that goes through `wp_mail()` — through [Emailit](https://emailit.com)'s API instead of the site's default mail transport.
...
<ExpandableGroup>
  ...
  <Expandable title="Icon" default-open="false">
    ...
    <Steps>
      <Step title="Declaration" icon="code" title-type="p">
        ```php
        ['id' => 'brand_icon', 'label' => 'Icon', 'type' => 'icon']
        ```
      </Step>
      <Step title="Usage" icon="play" title-type="p">
        ```php
        ...
        echo Lucide::render(OptionsPage::get('brand_icon'), ['class' => 'w-6 h-6']);
        ```
      </Step>
    </Steps>
  </Expandable>
  ...
  <Expandable title="Gradient Text" default-open="false">
    Ordered list of `{text, highlighted}` segments — same shape and admin UI as the Metabox `gradient_text` field above, backed by `wp_options` instead of post meta.

    <Steps>
      <Step title="Declaration" icon="code" title-type="p">
        ```php
        ['id' => 'site_tagline', 'label' => 'Tagline', 'type' => 'gradient_text']
        ```
      </Step>
      <Step title="Usage" icon="play" title-type="p">
        ```php
        $segments = json_decode(OptionsPage::get('site_tagline', '_taw_', '[]'), true);
  ...
  <Expandable title="HubSpot Form" default-open="false">
    HubSpot embed config — `{portal_id, form_id, region}` — same shape as the Metabox `hubspot_form` field above, for a site-wide default embed rather than a per-post one.

    <Steps>
      <Step title="Declaration" icon="code" title-type="p">
        ```php
        ['id' => 'default_hubspot_form', 'label' => 'Default HubSpot Form', 'type' => 'hubspot_form']
        ```
      </Step>
      <Step title="Usage" icon="play" title-type="p">
        ```php
        ...
        $config = OptionsPage::get('default_hubspot_form');
        echo Hubspot::render($config);
        ```
      </Step>
    </Steps>
  ...
  <Expandable title="Link" default-open="false">
    A URL, its text and “open in a new tab” as one value, same as the Metabox `link` field (`taw/core` v1.59.0+).

    ```php
    ['id' => 'footer_cta', 'label' => 'Footer link', 'type' => 'link']
    ```

    ```php
    echo TAW\Taw::option('footer_cta');   // the escaped <a>
    ```
  </Expandable>
</ExpandableGroup>
...
| `rest`      | *(none)*                        | `'private'` (in `/wp/v2/settings`, administrators) or `'public'` (also `GET /taw/v1/options/<id>`, anyone). See [Options pages over REST](/data-layer#options-pages-over-rest) |
...
| `GET` | `/taw/v1/icons` | Lucide icon search powering the `icon` field type. Requires `edit_posts`. Only registered when `Lucide::enable()` was called — see [Icon System](#icon-system). |
| `GET` | `/taw/v1/options/{page}` | Values of an options page with `rest: 'public'`, decoded. **Public, no login.** Only registered for such pages — see [Options pages over REST](/data-layer#options-pages-over-rest). |
...
| `GET` | `/taw/v1/bible/books` | Bible reader — see [Bible Reader Corpus](#bible-reader-corpus). Opt-in, public, rate-limited. |
...
`TAW\Core\Rest\RagChatEndpoint` runs a tool-calling chat turn against one or more named knowledge bases — see [Sovereign Hybrid-RAG Chatbot](#sovereign-hybrid-rag-chatbot) for the full picture.
...
<CodeGroup tabs="200 OK,429 Too Many Requests">
  ```json
  {
...
<ParamField body="message" param-type="string" required="true" deprecated="false">
  The user's message. 1–4000 characters.
</ParamField>

<ParamField body="history" param-type="array" required="false" deprecated="false">
...
`TAW\Core\Rest\BibleEndpoint` — read-only routes over an installed reference corpus. See [Bible Reader Corpus](#bible-reader-corpus) for the full picture. Only registered when `BibleEndpoint::enable()` has been called; all three routes `404` with `{"error": "No Bible corpus is installed."}` until `bin/taw corpus:install` has been run.
...
<ParamField query="slug" param-type="string" required="true" deprecated="false">
  Book slug (path parameter on the chapter route), e.g. `genesis`, `iohannes`.
</ParamField>

<ParamField query="q" param-type="string" required="true" deprecated="false">
...
<ParamField query="scope" param-type="string" required="false" deprecated="false">
...
<CodeGroup tabs="200 OK,404 Not Found">
  ```json
  {
    "book": {
...
`TAW\Core\Rest\CatechismEndpoint` — the same read-only posture as `bible`, one level deeper (part → section → chapter → question/answer) and explicitly scoped to one **edition** per request. See [Catechism Reader Corpus](#catechism-reader-corpus) for the full picture. Only registered when `CatechismEndpoint::enable()` has been called.
...
<ParamField query="edition" param-type="string" required="true" deprecated="false">
...
<CodeGroup tabs="200 OK,404 Unknown edition,404 Not installed">
  ```json
  {
...
Declare it exactly like any other Metabox or OptionsPage field — see the [Icon](#supported-field-types) entry above for the full declaration/usage example. The stored value is a bare icon name (e.g. `house`), sanitized with `sanitize_key()`.
...
`GET taw/v1/icons?search=&per_page=` powers the wp-admin picker (see [REST API](#rest-api) below) and is only registered once `Lucide::enable()` has been called. Requires `edit_posts` plus a valid `wp_rest` nonce, same as the other TAW REST routes.
...
- A `taw_media_folder` query param on the existing `wp/v2/media` route, for filtering by folder and for reassigning a file's folder (`PATCH wp/v2/media/<id>`).
...
<Columns cols="3">
  <Card title="Media → Folders" icon="folder-tree" horizontal={false}>
  ...
  <Card title="Classic List view" icon="list" horizontal={false}>
  ...
  <Card title="Grid view sidebar" icon="layout-panel-left" horizontal={false}>
...
<Card title="Content Interchange" href="/content-interchange" icon="arrow-left-right" horizontal="false">
...
- **scalar fields** → `register_post_meta()` with `show_in_rest`, the field's own sanitizer, and an `auth_callback` gated on `edit_post` for that post.
...
- **term and user fields** ([term](/data-layer#fields-on-terms) and [user](/data-layer#fields-on-users) fieldsets) → the same, on the term (`edit_term`) or the user (`edit_user`, `edit` context only).
- Options pages are **not** exposed here; a page opts in with [`rest`](/data-layer#options-pages-over-rest).

<Callout kind="info">
  This exposes field values over `wp/v2` for **headless front-ends and external integrations**. It does not add a mobile-app editing UI — classic metaboxes stay desktop-only; on-phone editing is the [Visual Editor](/taw-core#visual-editor). Opt out with `add_filter('taw_register_meta_in_rest', '__return_false')`.
</Callout>

{/* MARK: Sovereign Hybrid-RAG Chatbot */}
...
<Callout kind="alert">
  The LLM API key is deliberately **not** one of these fields — the options table is the wrong place for a secret (an options page can [publish its fields over REST](/data-layer#options-pages-over-rest)). Define it in `wp-config.php` instead:
...
See [`POST /taw/v1/chat`](#chat) under REST API for the request/response shape.

{/* MARK: Bible Reader Corpus */}
...
See [`bible`](#bible) under REST API for the full request/response shape of all three routes.
...
<Callout kind="info">
  `MysqlCatechismReader` reuses `MysqlBibleReader`'s `innodb_ft_min_token_size` short-word filtering from the start, rather than rediscovering the same production gap — see [Bible Reader Corpus](#bible-reader-corpus)'s note on that setting. `TAW\Core\Rest\CatechismEndpoint` resolves the reader the same SQLite-first, filterable way `BibleEndpoint` does, via the `taw_corpus_catechism_reader` filter.
</Callout>

See [`catechism`](#catechism) under REST API for the full request/response shape of every route.

{/* MARK: Translations */}

## Translations

taw-core's own strings (admin screens, field controls, form and password-gate messages, the data panel) use the **`taw-core`** text domain since v1.56.0, and its translations ship in the package (`languages/`). They load on their own in the site's language, so you don't call anything.

- **Spanish (`es_MX`)** is bundled, starting with the strings TAW sites had already translated.
- **Your theme's translation wins.** Before v1.56.0 these strings used the theme's `taw-theme` domain, so sites translated some of them in their own `languages/es_MX.po`. When the theme translates a taw-core string, that wording is used; the bundled translation fills in the rest. To change a taw-core string's wording on one site, translate it in the theme's `.po`.
- Your theme's own strings keep your theme's domain: `__('Phone', 'taw-theme')` in `inc/options.php` is correct.

<Expandable title="Working on taw-core's translations" default-open="false">
  In the taw-core repository, no wp-cli or gettext needed:

  ```bash
  composer run i18n:pot       # languages/taw-core.pot from the PHP and the data panel's TSX
  ...
  composer run i18n:check     # CI: everything current, every call uses 'taw-core'
  ```

  To add a language, copy `taw-core.pot` to `taw-core-<locale>.po`, set its `Language` and `Plural-Forms` headers, translate it, and compile.
</Expandable>
...
| `code` | A stable, dot-namespaced key (`subsystem.event`, e.g. `form.email_delivery_failed`). This is the contract an automated consumer — an AI agent, or the [TAW Hub](/taw-hub-companion) — filters and alerts on. Treat shipped codes as stable. |
...
`TAW\Core\Log\LogReader` is the underlying reader (same `level` / `code`-prefix / `since` filters). On a site that has joined a fleet, the [`taw-hub-companion`](/taw-hub-companion) plugin exposes the same data over a signed `GET /wp-json/taw-hub/v1/logs` route so the Hub can report on the site without shell access.
...
<Columns cols="2">
  ...
  <Card title="TAW CLI" href="/taw-core-cli" icon="terminal" horizontal="false">
    Work with Symfony Console commands via `bin/taw`, including `make:block`, `export:block`, `import:block`, `inspect`, `fields:get`/`fields:set` for direct field read/write, and `sync` for automated framework-drift detection.
  </Card>
