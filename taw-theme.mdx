...
<Callout kind="info" collapsed="false">
  The Visual Editor is opt-in. Call `VisualEditor::enable()` before `Theme::boot()` to activate it — in scaffolds using `Theme::bootstrapFullSite()`, that means `inc/customizations.php`, not `functions.php` (which is framework-owned). See the [Visual Editor](/taw-core#visual-editor) section for the full API and template annotation helpers.
</Callout>

<Callout kind="info" collapsed="false">
  The `icon` field type is opt-in — call `Lucide::enable()` in `inc/customizations.php` before `Theme::boot()`. It's commented out by default in this theme's scaffold. See [Icon System](/taw-core#icon-system).
</Callout>

<Callout kind="info" collapsed="false">
  Media Folders is opt-in at the `taw/core` level, but this theme's `inc/customizations.php` scaffold calls `MediaFolders::enable()` by default, so every new taw-theme site has nestable Media Library folders active out of the box. See [Media Folders](/taw-core#media-folders).
</Callout>

<Callout kind="info" collapsed="false">
  The scaffold ships a site-owned `inc/security.php` (required from `inc/customizations.php`, never touched by `update-theme`) that locks down user enumeration: it calls `Hardening::hideUsersEndpoint()` and redirects logged-out `?author=N` probes. Edit or trim it per site. See [Security / Hardening](/taw-core#security--hardening).
</Callout>

<Callout kind="info" collapsed="false">
  `bin/taw` includes `content:export` / `content:import` / `content:diff` for moving a site's state — posts, TAW fields, options, terms, media, and optionally authorship, users, comments and settings — between environments, and wp-admin gets a **Tools → TAW Data** screen. Import is always a dry-run first, with an automatic rollback snapshot. `content:export --migrate` plus `content:import --yes` is a full TAW-site state transfer. See [Content Interchange](/content-interchange).
</Callout>

<Callout kind="info" collapsed="false">
  The Sovereign Hybrid-RAG Chatbot is opt-in (since `taw/core` v1.30.0) — call `RagSettings::enable()` in `inc/customizations.php` before `Theme::boot()` (commented out by default, since it needs an LLM endpoint + `TAW_RAG_API_KEY` most sites won't have). This theme's `Blocks/Chatbot` widget checks `RagSettings::isEnabled()` before enqueuing/rendering itself, so it stays off automatically until enabled. See [Sovereign Hybrid-RAG Chatbot](/taw-core#sovereign-hybrid-rag-chatbot).
</Callout>

<Callout kind="info" collapsed="false">
  The Bible Reader Corpus is opt-in — call `BibleEndpoint::enable()` in `inc/customizations.php` before `Theme::boot()`. A developer-installed reference corpus (`bin/taw corpus:install`), not bundled with the theme scaffold. See [Bible Reader Corpus](/taw-core#bible-reader-corpus).
</Callout>
...
<ParamField body="ViteLoader::assetUrl($path)" param-type="string" required="true" deprecated="false">
...
<ParamField body="ViteLoader::isDevServerRunning()" param-type="bool" required="false" deprecated="false">
...
<ParamField body="Image::render($id, $size, $options = [])" param-type="string" required="true" deprecated="false">
...
<ParamField body="Image::preloadTag($id, $size)" param-type="string" required="true" deprecated="false">
...
- **Bottom row** — `primary` nav menu with hover dropdowns for items that have children, plus optional company email and phone from `OptionsPage`.
...
### Chatbot — site-wide Hybrid-RAG chat widget

`Blocks/Chatbot/` is a plain (presentational) `Block`, mounted site-wide rather than per-page — `header.php` enqueues its assets, `footer.php` renders it, both gated on the backend actually being enabled:
...
It talks only to `POST /wp-json/taw/v1/chat` — see [Sovereign Hybrid-RAG Chatbot](/taw-core#sovereign-hybrid-rag-chatbot) in the `taw-core` reference for the full backend picture (content-agnostic SQLite knowledge bases, embeddings, LLM tool-calling). Markdown replies are rendered with `marked` and sanitized with `DOMPurify` before any `x-html` binding.

<Callout kind="alert">
  **Opt-in** (since `taw/core` v1.30.0) — the widget only enqueues/renders once `RagSettings::enable()` has been called (commented out by default in `inc/customizations.php`, since the chatbot needs an LLM endpoint + `TAW_RAG_API_KEY` most sites won't have). Without it, `RagSettings::isEnabled()` returns `false` and the widget correctly stays off rather than rendering something that POSTs to a REST route that isn't registered.
</Callout>

<Callout kind="tip">
  Once enabled, all further configuration — LLM base URL/model, which knowledge bases exist, and whether anonymous visitors can chat — lives in wp-admin under **Settings → TAW Chatbot** (and its **Knowledge Bases** sub-page, where any `.sqlite` file can be uploaded). Nothing about the chatbot's behavior is configured in this theme.
</Callout>
...
<Callout kind="info">
  Adopting a new minor can add `bin/taw` commands. After `composer update taw/core`, run your scaffold's `bin/taw` sync (or copy the current `bin/taw` from the [taw-theme scaffold](https://github.com/Relmaur/taw-theme)) so new commands — for example `content:export` / `content:import` / `content:diff` for [Content Interchange](/content-interchange) — are registered.
</Callout>
...
Runs automatically in CI on every push/PR. Prefer fixing types over adding `ignoreErrors` entries — see the `wp-phpstan` skill in [WordPress/agent-skills](https://github.com/WordPress/agent-skills) for WordPress-specific typing patterns.
...
See the [AI Integration](/ai-integration) page for the full picture — skills, this safety model, `inspect`, WP-CLI, CI, and static analysis together.
...
`ViteLoader::isDevServerRunning()` is safe under WP-CLI — the dev-server probe only runs on `wp_enqueue_scripts`, which doesn't fire for most `wp` commands. For general WP-CLI operations beyond this framework's scope, see the `wp-wpcli-and-ops` skill in [WordPress/agent-skills](https://github.com/WordPress/agent-skills).
