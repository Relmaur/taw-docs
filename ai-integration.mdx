...
<Columns cols="2">
  ...
  <Card title="audit-seo" icon="search" horizontal="false">
    "audit SEO for page 42" → extracts a post's text-bearing copy via `php bin/taw seo:extract`, analyzes keyword presence, copywriting impact, and readability, then — only with explicit approval — writes rewrites back via `php bin/taw seo:inject`.
  </Card>
</Columns>
