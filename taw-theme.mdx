...
`Blocks/Chatbot/` is a plain (presentational) `Block`, mounted site-wide rather than per-page — `header.php` enqueues its assets, `footer.php` renders it:
...
<Callout kind="tip">
  All configuration — LLM base URL/model, which knowledge bases exist, and whether anonymous visitors can chat — lives in wp-admin under **Settings → TAW Chatbot** (and its **Knowledge Bases** sub-page, where any `.sqlite` file can be uploaded). Nothing about the chatbot's behavior is configured in this theme.
</Callout>
