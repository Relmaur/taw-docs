...
<Callout kind="info" collapsed="false">
  The Visual Editor is opt-in. Call `VisualEditor::enable()` before `Theme::boot()` in `functions.php` to activate it. See the [Visual Editor](/taw-core#visual-editor) section for the full API and template annotation helpers.
</Callout>
