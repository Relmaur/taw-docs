...
<Steps>
  ...
  <Step title="Enrol the site with the Hub" icon="link">
    With `taw/core` ≥ v1.23.0, mint a one-time enrolment token on the Hub (`php artisan fleet:enrol-token`) and run `php bin/taw hub:enroll --token=enrol_…` on the site — it `POST`s the site's identity to the Hub's `POST /api/fleet/enroll`, verifies the Hub's signed reply against `TAW_HUB_PUBLIC_KEY`, and reports the assigned `site_id`. On older `taw/core`, a signed `GET /wp-json/taw-hub/v1/health` returns `site_public_key` and `site_key_id` (also shown in an admin notice) — give those to the Hub operator to register by hand.
  </Step>
</Steps>

## Updating

From `0.2.0` the plugin self-updates from its **GitHub releases** (`Relmaur/taw-hub-companion`) — there is no per-site zip upload and no SSH step.

- Every site shows the standard **"Update available"** row in *Plugins*, and `wp plugin update taw-hub-companion` works.
- Unless `TAW_HUB_COMPANION_AUTO_UPDATE` is set to `false` in `wp-config.php`, the plugin opts itself into WordPress's **background auto-update cron** — a tagged release reaches the whole fleet within a cron cycle (~12 h) with nobody clicking anything.

<Callout kind="info">
  Releasing is `git tag vX.Y.Z && git push origin vX.Y.Z`: a CI workflow builds the plugin zip (dev files stripped, no `vendor/` — the plugin has an autoload fallback) and publishes it as the release asset the updater points at. The tag must match both the `Version:` header and `TAW_HUB_COMPANION_VERSION`. A site on a pre-`0.2.0` build has no updater — bring it to `0.2.0` once by hand, and it self-maintains after that.
</Callout>
...
- Enrolment needs a one-time token minted on the Hub. `php bin/taw hub:enroll` (`taw/core` ≥ v1.23.0) automates the round trip and verifies the signed reply; on older `taw/core` the operator registers the site's key by hand from `GET /health`.
