...
<Callout kind="info">
  `update-theme` refreshes framework skills but never deletes a skill your site authored for its own workflows. A site-authored skill is preserved across syncs as long as its `SKILL.md` frontmatter carries `owner: site` (framework skills carry `owner: taw`). See [update-theme](#update-theme) below.
</Callout>
...
<Columns cols="2">
  ...
  <Card title="figma-fidelity" icon="ruler" horizontal="false">
    "it doesn't look like the design" / "match Figma exactly" → the numbers-based fidelity pass: measures the live DOM against Figma's own sizes on both the desktop and mobile frames, closes the gaps, and reports drift. Encodes the recurring traps — unlayered `critical.scss` beating utilities, shared fluid spacing vs Figma's literal values, remapped `@theme` scales, Figma stretching photos, greedy text wrapping.
  </Card>

  <Card title="figma-image-crop-fix" icon="crop" horizontal="false">
    "the crop is off" → derives the exact pan/zoom (or stretch) treatment for an image from Figma's own generated CSS, and knows when a percentage transform is safe versus when to use `object-position`.
  </Card>
  ...
  <Card title="update-theme" icon="refresh-cw" horizontal="false">
    "update the theme" → copies a small, precisely-delimited set of framework-owned paths (`functions.php`, `bin/`, CI config) from the canonical `taw-theme` repo. Direct file sync, no git merge, no shared history required — never touches your `Blocks/`, templates, or content. The skill directories (`.claude/skills/`, `.agents/skills/`) are reconciled per skill: framework skills are refreshed, retired ones removed, and any skill your site authored (`owner: site`) is left untouched.
  </Card>
  ...
  <Card title="export-static" icon="globe" horizontal="false">
  ...
  <Card title="audit-seo" icon="search" horizontal="false">
