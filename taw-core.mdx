...
<ParamField query="q" param-type="string" required="true" deprecated="false">
  Search query — treated as one literal phrase (internal quotes are escaped), not raw FTS5 `MATCH` syntax.
</ParamField>
...
$reader->searchVerses('en el principio');  // FTS5 phrase search over verse text, <mark>-highlighted excerpt
$reader->searchNotes('creación');          // FTS5 phrase search over Straubinger's own footnote commentary
```

`chapter()` returns `verses`/`sections`/`notes` as three flat, chapter-scoped lists rather than an interleaved rendering shape — where a heading sits relative to a verse, or how a note marker anchors into verse text, is presentation, and stays the consuming theme's decision.
