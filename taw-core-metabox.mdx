...
| `readonly` | `bool` | Render as non-interactive text; never written on save (see [Readonly fields](#readonly-fields)) |
...
<ExpandableGroup>
  ...
  <Expandable title="icon" default-open="false">
    ...
    <Callout kind="tip">
      Opt-in — call `TAW\Core\Icons\Lucide::enable()` in `inc/customizations.php` before `Theme::boot()` (block themes: before `Boot::data()`), or this field renders a notice instead of the picker. Full reference: `taw-core` README § "Icon System".
    </Callout>
    ...
    echo Lucide::render(Metabox::get($postId, 'feature_icon'), ['class' => 'w-6 h-6']);
    ```
  </Expandable>

  <Expandable title="gradient_text" default-open="false">
    Ordered list of `{text, highlighted}` segments — for a gradient heading where the highlighted run isn't always the trailing segment. Alpine-only admin segment editor, no drag-reorder — use `repeater` instead when row order matters.
  ...
  <Expandable title="hubspot_form" default-open="false">
    HubSpot embed config — `{portal_id, form_id, region}`.

    <Callout kind="tip">
      No `enable()` gate needed. `region` defaults to `na1` when blank. Full reference: `taw-core` README § "HubSpot Form".
    </Callout>
...
Set `'readonly' => true` on any field whose value is authoritatively owned by something other than the wp-admin form — an external `.md`-file sync pipeline, a computed value — so the metabox stops implying it's editable there.

```php
['id' => 'case_client', 'label' => 'Client', 'type' => 'text', 'readonly' => true],
```

A small lock icon appears next to the field's label (with a "Managed externally — read-only" tooltip), so the field reads as intentionally locked rather than as a bug in the admin UI.

<Callout kind="info" collapsed="false">
  This is enforced on both ends, not just visually. No `<input>`/`<select>`/`<textarea>` is rendered — there's nothing for devtools to re-enable and submit — and `Metabox::save()` skips the field entirely, so a forged `$_POST` key is ignored regardless of what the rendered markup looked like.
</Callout>

`readonly` composes with `group` and `repeater` rather than being its own field type:

- On a `group` field, it propagates to every sub-field.
- On a `repeater` field, it disables Add/Remove/reorder for the whole row list — not just the sub-field values — and propagates to every sub-field in every row. A synced list whose *membership* could still be edited would be just as misleading as an editable input. A `readonly` sub-field inside an otherwise-editable repeater is also supported and enforced per row.

<Callout kind="alert" collapsed="false">
  Not currently enforced by `TAW\Core\Rest\VisualEditorEndpoint` or the `fields:set`/`seo:inject` CLI commands — those are separate write paths that don't consult this flag yet.
</Callout>
...
<Callout kind="tip">
  Since `taw/core` v1.57.0 there's also a typed API: `$this->fields($postId)->field('hero_image')->image()->url()` in a block, or `Taw::post()` anywhere, with values escaped when echoed. See [Reading fields](/reading-fields).
</Callout>
