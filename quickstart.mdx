...
<Steps>
  ...
  <Step title="Enable automated framework updates (one-time GitHub setting)" icon="refresh-cw" title-type="p">
    Your new project ships with `.github/workflows/framework-sync.yml` from the first commit — a weekly check that bumps `taw/core`, syncs framework-owned scaffold files, and opens a pull request if anything changed. It needs one manual setting enabled on the new repo:
