...
<Callout kind="info" collapsed="false">
  `bin/taw` includes `content:export` / `content:import` / `content:diff` for moving posts, TAW fields, options, terms and referenced media between environments, and wp-admin gets a **Tools → TAW Data** screen. Import is always a dry-run first, with an automatic rollback snapshot. See [Content Interchange](/taw-core#content-interchange).
</Callout>
