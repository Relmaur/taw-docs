...
- **`layers.content`**: a map of post type to a level name or a rule:
  ...
  - `allowBound` (taw/core v1.62.0+): blocks clients may add only bound to a TAW field. See [Block Bindings](/block-bindings#field-only-blocks-in-locked-down-post-types).
...
| Unbound `allowBound` blocks | **Server:** a save that adds more unbound ones than the post had is rejected with a 400 (`taw_editing_block_not_bound`). The editor also locks saving with a notice |
...
- **Field-only blocks:** a content rule's `allowBound` lets clients add some blocks only bound to a TAW field. See [Block Bindings](/block-bindings#field-only-blocks-in-locked-down-post-types).
