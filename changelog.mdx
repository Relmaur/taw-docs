...
<Update label="2026-09-26" description="taw/core v1.67.0–v1.67.1 / taw-gutenberg 0.3.26–0.3.27 / taw-theme v1.12.27–v1.12.28: conditions in the editor." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Conditions in the editor

Tagged taw/core **v1.67.0** and **v1.67.1**, TAW Gutenberg **v0.3.26–v0.3.27** and taw-theme **v1.12.27–v1.12.28**. See [Conditions](/conditions).

- **Show only when…** in the TAW data popup's Expression tab and in a value's editor: a rule builder (value · comparison · what to compare with; all or any; groups) with **Otherwise show** text.
- **Visibility** tab in the popup, and **TAW visibility** in every block's sidebar: show a whole block only when rules hold.
- A live answer, **Shown / Hidden for this post**; conditional blocks and values are marked in the canvas.
- TAW Gutenberg's book template hides the subtitle when there is none, shows "First published …" only with a year, and adds a **Buy this book** button (a new `book_buy_link` field) that shows only with a link.
- v1.67.1: in the Site Editor, conditions are checked against the latest post of the template's type.

</Update>

<Update label="2026-09-26" description="taw/core v1.66.0 / taw-gutenberg 0.3.25 / taw-theme v1.12.26: conditions (server side)." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Conditions

Tagged taw/core **v1.66.0**, TAW Gutenberg **v0.3.25** and taw-theme **v1.12.26**. A chip, a block's bound text, or a **whole block** can show only when rules hold:

- “First published **1965**” only when the year is set; a **Buy** button only when the buy link isn't empty; a “New” badge only for posts from the last 30 days.
- 23 comparisons: empty / not empty, text (equals, contains, starts or ends with), lists (is one of, has), numbers (greater or less than, between), dates (before, after, on, between; with `today` or `-30 days`), yes / no.
- Rules combine with **All** or **Any**, one group deep. A hidden chip can show other text instead.
- New values: `@viewer.logged_in`, `@viewer.role`, `@date.today`, `@date.now`.
- Each theme gains one hook; content without conditions renders as before.

This release is the server side. The rule builder in the TAW data popup comes next.

</Update>

<Update label="2026-09-26" description="taw/core v1.65.1 / taw-gutenberg 0.3.24 / taw-theme v1.12.25: chip popover fixes." tags={["taw/core", "taw-gutenberg", "taw-theme", "fix"]}>

## September 2026 — Chip popover fixes

Tagged taw/core **v1.65.1**, TAW Gutenberg **v0.3.24** and taw-theme **v1.12.25**.

- A chip's popover **closes** after **Save** or **Refresh**, and on Escape or a click elsewhere. Before, it flickered and stayed open.
- Removing a chip (**Remove**, Backspace or Delete) no longer throws an error in the editor.

</Update>

<Update label="2026-09-26" description="taw/core v1.63.0–v1.65.0 / taw-gutenberg 0.3.20–0.3.23 / taw-theme v1.12.22–v1.12.24: dynamic tags and expressions." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Dynamic tags and expressions

Tagged taw/core **v1.63.0**, **v1.64.0** and **v1.65.0**. See [Dynamic tags and expressions](/dynamic-tags).

- **Values inside a sentence:** “Published on **September 26, 2026** by **Frank Herbert**”. Each value is an atomic chip in the editor and always live on the front end, with the same privacy rules as Block Bindings.
- **Expressions:** text and values in one string, e.g. `Published on @post.date.format('F j, Y') by @post.author`, with `format`, `upper`, `lower`, `default` and `truncate`.
- **The TAW data popup** (the block toolbar's database button): a **Fields** tab grouped by fieldset and an **Expression** tab with suggestions, a live preview and error hints. Insert **inline** or use as the **block text**.
- TAW Gutenberg's book template shows “Published on …” with an expression.
- Hook lists gain three lines in total; content without tags renders exactly as before.

</Update>

<Update label="2026-09-25" description="taw/core v1.60.0–v1.62.0 / taw-gutenberg 0.3.16–0.3.18 / taw-theme v1.12.19–v1.12.21: Block Bindings." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Block Bindings: TAW fields in core blocks

Tagged taw/core **v1.60.0**, **v1.61.0** and **v1.62.0**, TAW Gutenberg **v0.3.16–v0.3.18** and taw-theme **v1.12.19–v1.12.21**. See [Block Bindings](/block-bindings).

- **v1.60.0:** a `taw/field` Block Bindings source. Paragraphs, headings, list items, buttons, images and post dates can show post, option, term and author fields, in templates, patterns and Query Loops too. Fields can opt out with `"bindings": false`; user fields opt in with `"bindings": true`.
- **v1.61.0:** the editor side. Bound blocks preview their real value, a **TAW field** toolbar button (and ⋮ → **Connect to TAW field…**) binds a block in one pick, and the **Attributes** panel lists TAW fields.
- **v1.62.0:** [editing policies](/editing-policies) gain `allowBound`: blocks clients may add only bound to a field, offered as **Field text**, **Field image**… in the inserter.
- TAW Gutenberg ships `templates/single-book.html`, built from bound core blocks, as the reference.
- Both themes' hook lists gain three lines in total; nothing renders differently until a block uses a binding.

</Update>

<Update label="2026-09-25" description="taw/core v1.59.2 / taw-gutenberg 0.3.15 / taw-theme v1.12.18: fixes from the first fleet upgrade." tags={["taw/core", "taw-gutenberg", "taw-theme", "fix"]}>

## September 2026 — Fixes from the first fleet upgrade

Tagged taw/core **v1.59.2**, TAW Gutenberg **v0.3.15** and taw-theme **v1.12.18**. Seven TAW sites were upgraded to v1.59.1 with the [upgrade guide](/help-center/guides/upgrading-taw-core), and these are the problems they found:

- **taw-core's Spanish translation now loads in classic themes.** From v1.56.0 to v1.59.1 only a site's own `.po` translated taw-core text.
- **`php bin/taw wp db …` works under Local**, and `bin/taw wp` tells you when the Local site is stopped.
- **`bin/taw sync` keeps a site's own skill** when a framework skill with the same name appears, and reports the clash.
- **Metabox and options-page tabs work by keyboard.** They look the same.
- **`UPGRADING.md`** has clearer checks:
  - a command that finds group values stored under the wrong key, and how to fix them;
  - form checks that don't send email;
  - notes on the optional Reactiph and chatbot starter features.

</Update>

<Update label="2026-09-25" description="taw/core v1.59.0 / taw-gutenberg 0.3.13 / taw-theme v1.12.16: a link field type." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — The link field

Tagged taw/core **v1.59.0**, TAW Gutenberg **v0.3.13** and taw-theme **v1.12.16**.

- **`link`** is a new field type: a URL, its text and “open in a new tab”, edited together and stored as one value. It works in metaboxes, options pages, repeater rows, term and user screens and the [data panel](/data-panel). Relative paths like `/contact` work, and `required` means a URL.
- `echo Taw::post()->field('cta')` prints the escaped `<a>`; `->link()` gives its parts. See [Reading fields](/reading-fields).
- Existing fields render and save exactly as before.

</Update>

<Update label="2026-09-25" description="taw/core v1.58.0 / taw-gutenberg 0.3.12 / taw-theme v1.12.15: typed reads for terms, users and options." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Typed reads everywhere

Tagged taw/core **v1.58.0**, TAW Gutenberg **v0.3.12** and taw-theme **v1.12.15**.

- `Taw::term()` (the queried term when called without an ID), `Taw::user()`, `Taw::option('company_phone')` and `Taw::options('site')` read fields the same way as `Taw::post()`. See [Reading fields](/reading-fields).

</Update>

<Update label="2026-09-25" description="taw/core v1.57.0 / taw-gutenberg 0.3.11 / taw-theme v1.12.14: typed, escaped field reads with Taw::post()." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Taw::post()

Tagged taw/core **v1.57.0**, TAW Gutenberg **v0.3.11** and taw-theme **v1.12.14**.

- **`TAW\Taw::post()->field('x')`** returns a typed value: `->image()`, `->rows()`, `->posts()`, `->bool()`, `->text()`, and `echo` escaped for the field's type. In a MetaBlock, `$this->fields($postId)` does the same. See [Reading fields](/reading-fields).
- Nothing throws: a missing post or field reads as empty. The existing helpers are unchanged.
- Updating an older site? See [Upgrading taw/core on an existing site](/help-center/guides/upgrading-taw-core).

</Update>

<Update label="2026-09-25" description="taw/core v1.56.0 / taw-gutenberg 0.3.10 / taw-theme v1.12.13: taw-core has its own text domain and ships its own translations." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — taw-core translations

Tagged taw/core **v1.56.0**, TAW Gutenberg **v0.3.10** and taw-theme **v1.12.13**.

- **Every taw-core string uses the `taw-core` text domain.** Before, most used the theme's `taw-theme` domain, and the form and password-gate messages used a `taw` domain that nothing loaded, so they were always English.
- **Translations ship with the package** (`languages/`) and load on their own in the site's language. Spanish (`es_MX`) starts with the strings TAW sites already translated. The data panel gets them too.
- **Your theme's translation wins.** When a theme's `languages/es_MX.po` translates a taw-core string, that wording keeps showing, so no site loses or changes a translation. See [Translations](/taw-core#translations).
- One visible change on Spanish sites: the form message "%s is required." is now translated.

</Update>

<Update label="2026-09-25" description="taw/core v1.55.0 / taw-gutenberg 0.3.9 / taw-theme v1.12.12: options pages can be exposed over REST, per page." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Options pages over REST

Tagged taw/core **v1.55.0**, TAW Gutenberg **v0.3.9** and taw-theme **v1.12.12**.

- **`rest: "private"`** adds an options page's values to `/wp/v2/settings`, for administrators, with the page's own validation and sanitizing.
- **`rest: "public"`** also serves them read-only at `GET /wp-json/taw/v1/options/<page id>`, to anyone. Use it only for pages with nothing private on them.
- Pages without `rest` expose nothing, as before. See [Options pages over REST](/data-layer#options-pages-over-rest).

</Update>

<Update label="2026-09-25" description="taw/core v1.54.0 / taw-gutenberg 0.3.8 / taw-theme v1.12.11: fieldsets on user profiles." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Fields on users

Tagged taw/core **v1.54.0**, TAW Gutenberg **v0.3.8** and taw-theme **v1.12.11**.

- **`"on": ["user"]`** shows a fieldset on Profile, Edit User and Add New User, with every field type, conditions and validation. See [Fields on users](/data-layer#fields-on-users).
- Over REST, user fields appear only to people who can edit that user. They never show on an author's public `/wp/v2/users/<id>`.
- Content snapshots carry them with `--with-users`.

</Update>

<Update label="2026-09-25" description="taw/core v1.53.1 / taw-gutenberg 0.3.7 / taw-theme v1.12.10: unchecking a checkbox hides its conditional fields again." tags={["taw/core", "taw-gutenberg", "taw-theme", "fix"]}>

## September 2026 — Checkbox conditions

Tagged taw/core **v1.53.1**, TAW Gutenberg **v0.3.7** and taw-theme **v1.12.10**.

- **Fix:** in the metabox form, unchecking a checkbox now hides the fields whose conditions depend on it, at the top level and inside repeater rows. Saving was already correct.

</Update>

<Update label="2026-09-25" description="taw/core v1.53.0 / taw-gutenberg 0.3.6 / taw-theme v1.12.9: fieldsets on taxonomy terms." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Fields on terms

Tagged taw/core **v1.53.0**, TAW Gutenberg **v0.3.6** and taw-theme **v1.12.9**.

- **`"on": ["term:genre"]`** shows a fieldset on a taxonomy's Add and Edit screens, alone or next to post types, with every field type, conditions and validation. See [Fields on terms](/data-layer#fields-on-terms).
- Read values with `Metabox::get_term()` or `Metabox::term($id)`. They're in REST on the term, and content snapshots carry them.
- Under the hood, the metabox engine now reads and writes through a storage context (post, term or user meta). Post metaboxes render and save byte-for-byte as before.

</Update>

<Update label="2026-09-25" description="taw/core v1.52.1 / taw-gutenberg 0.3.5 / taw-theme v1.12.8: content import finds drafts and private posts by slug from the CLI." tags={["taw/core", "taw-gutenberg", "taw-theme", "fix"]}>

## September 2026 — Import matches drafts

Tagged taw/core **v1.52.1**, TAW Gutenberg **v0.3.5** and taw-theme **v1.12.8**.

- **Fix:** `bin/taw content:import` runs without a user, so it couldn't see drafts and private posts, and planned them as new posts. It now matches them by slug, and references to them (`parent`, `page_on_front`, comments) resolve.

</Update>

<Update label="2026-09-25" description="taw/core v1.52.0 / taw-gutenberg 0.3.4 / taw-theme v1.12.7: qualified field ids, and content snapshots carry fields with any prefix." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature", "fix"]}>

## September 2026 — Qualified field ids

Tagged taw/core **v1.52.0**, TAW Gutenberg **v0.3.4** and taw-theme **v1.12.7**.

- **Every field also has a qualified id**, `"{fieldset}.{field}"`. REST meta, content export and import, the data panel, the visual editor and `fields:get` / `fields:set` look fields up per post type, so two fieldsets can share a field id and each keeps its own type and sanitizer. See [Field ids](/data-layer#defaults-and-rules).
- **Content interchange 1.2:** fields with a prefix other than `_taw_` are exported, keyed by their meta key. `_taw_` fields keep their bare-id keys, and the importer still reads 1.0 and 1.1 files. See [Content Interchange](/content-interchange).
- **Fix:** `fields:set` and the importer wrote group sub-fields to the wrong meta key (`_taw_{sub}` instead of `_taw_{group}_{sub}`).

</Update>

<Update label="2026-09-25" description="taw/core v1.51.1 / taw-gutenberg 0.3.3 / taw-theme v1.12.6: icon fields work in block themes." tags={["taw/core", "taw-gutenberg", "taw-theme", "fix"]}>

## September 2026 — Icon fields in block themes

Tagged taw/core **v1.51.1**, TAW Gutenberg **v0.3.3** and taw-theme **v1.12.6**.

- **`Boot::data()` now registers the icon search** (`/taw/v1/icons`) when `Lucide::enable()` has been called. Before, only `Theme::boot()` did, so `icon` fields in TAW Gutenberg showed a picker that couldn't load icons, in the metabox and in the [data panel](/data-panel).
- Nothing changes for sites that don't call `Lucide::enable()`, and taw-theme behaves exactly as before.

</Update>

<Update label="2026-09-25" description="taw/core v1.51.0 / taw-gutenberg 0.3.2 / taw-theme v1.12.5: the data panel shows TAW fields in a block-editor sidebar, off by default." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature", "fix"]}>

## September 2026 — The data panel

Tagged taw/core **v1.51.0**, TAW Gutenberg **v0.3.2** and taw-theme **v1.12.5**.

- **The data panel** shows fieldsets in a “TAW Data” sidebar in the block editor instead of as metaboxes under the canvas. See [Data panel](/data-panel).
  - It's **off by default**. Switch it on per fieldset (`"ui": "panel"`), for the site (a `settings` schema file with `fieldsetUi`), or per install (`TAW_DATA_UI` in `wp-config.php`).
  - Every field type works, including groups, repeaters (with nested ones) and conditions. Values are stored exactly as the metabox stores them.
  - `wysiwyg` fields open a small block editor in a modal and keep the classic editor's storage format, so templates render them as before.
  - The server checks `required`, `validate` callbacks and read-only fields on block-editor saves. In the editor, saving is locked while a required field is empty.
- **Fix: repeater values written outside the metabox form** (REST, the visual editor, `fields:set`, `content:import`) no longer lose nested repeaters, or `files` and multiple `post_select` values, inside rows.
- **Fix: REST writes of `gradient_text` and `hubspot_form`** are cleaned the way the metabox cleans them (for example, an empty HubSpot region becomes `na1`).
- Both themes only update their taw/core lock. The panel stays off in both, and their hooks are unchanged.

</Update>

<Update label="2026-09-25" description="taw/core v1.50.0 / taw-gutenberg 0.3.1 / taw-theme v1.12.4: themeBlocks keeps a theme's own blocks available when the editor is locked down." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Theme blocks in locked-down editors

Tagged taw/core **v1.50.0**, TAW Gutenberg **v0.3.1** and taw-theme **v1.12.4**.

- **`themeBlocks`** is a new editing-policy key: your theme's own blocks, as names or globs (`["taw-gutenberg/*"]`).
  - They're added to every allow list, the curated one included, so clients at `guided` and above can still insert them.
  - Levels that allow every block are unchanged, and it still applies when `TAW_EDITING_PRESET` sets the level.
  - See [Editing policies](/editing-policies).
- **TAW Gutenberg 0.3.1** ships `themeBlocks: ["taw-gutenberg/*"]`, so its Callout block stays available to locked clients.
- **Tools → TAW Editing** shows the theme blocks and warns when a pattern matches no registered block.

</Update>

<Update label="2026-09-25" description="taw/core v1.49.0–v1.49.1 / taw-gutenberg 0.3.0 / taw-theme v1.12.3: a shared Vite adapter, and custom blocks with Vite and TypeScript in TAW Gutenberg." tags={["taw/core", "taw-gutenberg", "taw-theme", "feature"]}>

## September 2026 — Vite for block themes

Tagged taw/core **v1.49.0** and **v1.49.1**, TAW Gutenberg **v0.3.0** and taw-theme **v1.12.3**.

- **`TAW\Core\Assets\Vite`** (v1.49.0) is a Vite adapter any TAW theme can use:
  - `script()`, `style()` and `url()`, plus `block()`, which loads a block.json's `file:` assets through Vite;
  - scripts load as ES modules and keep their translations;
  - without a build, the site still renders and administrators see what's missing.
- **One dev-server check for both loaders:** the check that only trusts the project's own `dist/hot` file now lives in `Assets\DevServer`, and the classic `ViteLoader` uses it too. taw-theme behaves exactly as before.
- **Shared Vite config** in `vendor/taw/core/resources/vite/taw-vite.mjs`: `wordpressExternals()` (use the React and `@wordpress/*` copies WordPress already loads), `hotFile()` and `phpReload()`. v1.49.1 makes it tree-shakeable, so a block keeps only the names it imports (the example block went from 16.56 kB to 1.71 kB).
- **TAW Gutenberg 0.3.0** builds custom blocks with Vite and TypeScript, and ships an example **Callout** block. `create-project` builds them for you. See [TAW Gutenberg](/taw-gutenberg#build-custom-blocks).

</Update>

<Update label="2026-09-24" description="taw/core v1.45.0–v1.48.0 / taw-gutenberg 0.2.0–0.2.1: editing policies lock the block editor down for a client, layer by layer, with presets from open to locked." tags={["taw/core", "taw-gutenberg", "feature"]}>

## September 2026 — Editing policies

Tagged **v1.45.0** to **v1.48.0**, with TAW Gutenberg **v0.2.0** and **v0.2.1**. An editing policy
decides how much a client can change in the block editor, in four layers: page content, site
structure, design tokens and editor features. See [Editing policies](/editing-policies).

- **Presets:** `open` → `guided` → `structured` → `locked`. Override any layer or single setting in `taw-schema/editing.json` (new JSON kind `editing`), with `Schema::editing()` in PHP, or per post type with `PostType::editing()`. `schema:validate` checks the file.
- **Per install:** `TAW_EDITING_PRESET` in `wp-config.php` picks the level. `TAW_EDITING_BYPASS_USERS` names who stays unlocked, even when clients are Administrators. `TAW_EDITING_OFF` turns everything off.
- **Enforced on the server:** a save that adds a block the policy doesn't allow gets a 400, and writes to locked templates, template parts, Global Styles or Navigation get a 403. At `locked`, the Site Editor is hidden and refused.
- **Content-only pages on WordPress 7.1** (v1.48.0): the editor now ignores a page-level `contentOnly` lock, so taw/core locks the layout and switches each block to content-only editing with a small script.
- **Tools → TAW Editing** shows the policy in effect, whether you bypass it, and warnings.
- **Opt-in:** the policy applies only when a theme calls `\TAW\Core\Boot::editing()`. TAW Gutenberg does, and ships the `open` preset. taw-theme doesn't, and its hooks are unchanged.

</Update>

<Update label="2026-09-23" description="taw-theme v1.12.1: composer create-project no longer prints a warning about a missing .env.example file." tags={["taw-theme", "fix"]}>

## September 2026 — taw-theme install fix

Tagged **v1.12.1**. `composer create-project taw/theme` no longer prints a `copy()` warning. The install script tried to copy `.env.example` to `.env`, but the theme ships no such file and nothing reads a `.env`, so the step is gone.

</Update>

<Update label="2026-09-23" description="taw-create 0.1.0: one command starts either TAW theme. composer create-project taw/create asks for taw-theme or taw-gutenberg." tags={["taw-create", "taw-gutenberg", "feature"]}>

## September 2026 — One installer for both TAW themes

**taw-create 0.1.0** is a new installer package. Composer can't pass arguments to a project's scripts, so the choice comes from a prompt, or from `TAW_STARTER` when there's no terminal:

```bash
cd wp-content/themes
composer create-project taw/create my-theme \
  --repository='{"type":"vcs","url":"https://github.com/Relmaur/taw-create"}'
```

- **Pick a theme:** `taw-theme` (classic, the default when non-interactive) or `taw-gutenberg` (block theme). `TAW_STARTER=theme|gutenberg` skips the question, and `TAW_STARTER_VERSION` pins a version.
- **Safe by design:** it never touches a folder that already has other files in it. A failed install leaves your folder as it was, and no shell is involved.
- **TAW Gutenberg** is now tagged **v0.1.0**, with a GitHub Release, so it installs as a stable version.
- The direct commands (`composer create-project taw/theme …`, `taw/gutenberg …`) still work.

</Update>

<Update label="2026-09-23" description="taw/core v1.44.0 / taw-theme v1.12.0 / taw-gutenberg 0.1.0: define your data model as JSON in taw-schema/, validate it without WordPress, and meet TAW Gutenberg, the new block theme." tags={["taw/core", "taw-theme", "taw-gutenberg", "feature"]}>

## September 2026 — JSON schema definitions, and TAW Gutenberg

Tagged **v1.44.0**, with a paired `taw-theme` release tagged **v1.12.0**. It covers the whole data-layer
series (v1.42.0–v1.44.0) plus the branding and PHP-requirement cleanups below.

- **JSON definitions:** post types, taxonomies, fieldsets and options pages can live as one JSON file each in a `taw-schema/` folder (child theme, parent theme, or `wp-content/`). PHP definitions outrank every file. An invalid file is skipped with a notice. See [Data layer](/data-layer).
- **`php bin/taw schema:validate`** checks those files **without WordPress**, pointing at each problem (`/fields/2/type`). `resources/schema/taw-schema-1.0.json` gives editors autocomplete.
- **TAW Gutenberg 0.1.0:** a new block (full site editing) theme that uses only taw/core's data layer. See [TAW Gutenberg](/taw-gutenberg).
- **Scope:** taw/core is now documented as serving TAW sites only (taw-theme, taw-gutenberg and their child themes).
- **taw-theme:** registers `schema:validate` in `bin/taw`. The README and `style.css` no longer spell out “TAW”, and both now state the real PHP requirement (8.2+).

</Update>

<Update label="2026-09-23" description="taw/core v1.43.0: a schema registry — define post types, taxonomies, fieldsets and options pages in PHP; they compile into the existing Metabox and OptionsPage engines." tags={["taw/core", "feature"]}>

## September 2026 — Schema registry

Tagged **v1.43.0**. Define your content model in one place:

```php
add_action('taw_schema_register', function (\TAW\Core\Schema\Registry $schema): void {
    $schema->add(Schema::postType('book')->labels('Book', 'Books'));
    $schema->add(Schema::taxonomy('genre')->for('book')->labels('Genre', 'Genres'));
    $schema->add(Schema::fieldset('book_details')->on('book')->fields([Field::text('book_author')]));
});
```

Fieldsets become regular `Metabox`es and options pages become `OptionsPage`s, so storage, the admin UI, REST meta and content export behave exactly like hand-written ones. Post types default to `public` + `show_in_rest`, and **always** get `custom-fields` support (WordPress hides registered meta from REST without it). Invalid keys are rejected up front, field-id collisions are reported, and rewrite rules flush once, on the next admin request, when post types change.

</Update>

<Update label="2026-09-23" description="taw/core v1.42.0: Boot::data(), a data-only entry point for TAW block themes. Performance optimizations now register on boot, not at autoload." tags={["taw/core", "feature"]}>

## September 2026 — Boot::data() and a data layer for block themes

Tagged **v1.42.0**. `taw/core` is now split into a **data layer** (fields, REST meta, content interchange) and the **classic-theme toolkit** (PHP blocks, Vite, Alpine, performance, visual editor):

- **`\TAW\Core\Boot::data()`** boots only the data layer, for block themes. `Theme::boot()` calls it too, so classic sites are unchanged.
- **Behavior note:** Performance optimizations (which dequeue the block library CSS, among other things) used to register the moment Composer loaded the package. They now register in `Theme::boot()` / `bootstrapFullSite()`. taw-theme sites see no difference (verified: identical hooks, in identical order). If a setup relied on the old behavior without ever calling `boot()`, define `TAW_PERFORMANCE_AUTOLOAD` as `true` in `wp-config.php`.
- `Framework::url()` resolves package assets from a child theme too. `dump()`/`dd()` no longer clash with other libraries. `ext-pdo_sqlite` is now optional (only the RAG and corpus features use it).

</Update>

<Update label="2026-09-20" description="taw/core v1.41.0 / taw-theme v1.11.14: fix — the documented Metabox 'tabs' option never rendered; a tabbed metabox now shows its tab UI instead of a flat field list." tags={["taw/core", "taw-theme", "bugfix"]}>

## September 2026 — Metabox tabs now render

`Metabox` accepted a `tabs` config (documented in the README and on the Metabox pages), but `render()` never called `render_tabs()` — it was dead code, so a tabbed metabox silently rendered as a flat field list. Tagged **v1.41.0**, with a paired `taw-theme` release tagged **v1.11.14**.

`render()` now resolves each tab's fields and renders the tab UI. Fields no tab lists stay visible above the tab bar rather than being dropped (saving walks every field, so an unposted field would be blanked); a tab whose `fields` match nothing is skipped, and a metabox with no usable tab renders flat. Every tab's inputs stay in the DOM — only the active panel is shown — so the metabox still saves as one form and conditional fields keep working.

**Behavior note:** a site whose Metabox already carried a `tabs` config now gets the tab UI it configured instead of a flat list.

</Update>

<Update label="2026-09-20" description="taw/core v1.40.0 / taw-theme v1.11.13: Form::register() gains per-form 'class' / 'button_class' options and emits field width as a --taw-span custom property instead of an inline grid-column. taw-theme: new figma-fidelity and figma-image-crop-fix skills." tags={["taw/core", "taw-theme", "feature"]}>

## September 2026 — Form styling hooks + Figma fidelity skills

Tagged **v1.40.0**, with a paired `taw-theme` release tagged **v1.11.13**. Flagged while pixel-matching a Figma Contact form: the submit button was hardcoded `taw-btn taw-btn-primary` with no per-form hook, and per-field width was an inline `grid-column: span N` that no stylesheet rule could beat without `!important`.

```php
Form::register([
    'id'           => 'contact',
    'class'        => 'contact-form',   // appended to the <form>'s `taw-form`
    'button_class' => 'btn-black',      // appended to `taw-btn taw-btn-primary` (submit, and Next on multi-step)
    'fields'       => [...],
]);
```

Both options add to the built-in classes, never replace them. A field's `width` is now the `--taw-span` custom property, turned into `grid-column` by `form.css`, so a theme can override the column with an ordinary selector. **Behavior note:** every grid cell (fields, `html`, `heading`, `divider`) now collapses to full width below 640px — previously only fields did, and the rule needed `!important`.

**taw-theme** gains two canonical Claude Code skills distilled from a long Figma-exact pass on a client site: **`figma-fidelity`** (measure the live DOM against Figma's own numbers on desktop *and* mobile frames, close gaps, report drift) and **`figma-image-crop-fix`** (derive the exact pan/zoom or `object-fit: fill` stretch treatment for an image, including the `object-position` derivation for fluid containers). `AGENTS.md` § "CSS entry points" now documents that `critical.scss` is unlayered and beats Tailwind utilities — use modifier classes, not utilities, to vary above-the-fold blocks.

</Update>

<Update label="2026-09-15" description="taw/core v1.39.0 / taw-theme v1.11.12: Form::register() gains a submit_icon option — a Lucide icon name or raw SVG/HTML rendered after the submit button's label." tags={["taw/core", "taw-theme", "feature"]}>
...
<Update label="2026-09-15" description="taw/core v1.37.1 / taw-theme v1.11.11: fix — gradient_text destroyed the word-boundary space around a highlighted phrase sitting mid-sentence, its primary use case." tags={["taw/core", "taw-theme", "bugfix"]}>
...
<Update label="2026-09-15" description="taw/core v1.37.0 / taw-theme v1.11.10: fields:set/fields:get gain an 'options' scope for site-wide OptionsPage fields, plus two new field types — gradient_text (multi-segment gradient headings) and hubspot_form (HubSpot embed config)." tags={["taw/core", "taw-theme", "feature"]}>
...
- **`gradient_text`** — an ordered list of `{text, highlighted}` segments, replacing the old convention of a plain `heading` field plus a single always-trailing `highlight` field (which can't express a heading like "How can **we help**?", where the highlighted run isn't last). Render with `Metabox::renderGradientText($segments, $highlightClass)`.
- **`hubspot_form`** — `{portal_id, form_id, region}`, rendered via new `TAW\Core\Integrations\Hubspot::render()`/`isConfigured()`, for a HubSpot embed with a fallback to the site's native `Form` block when unconfigured.

</Update>

<Update label="2026-09-14" description="taw/core v1.33.1 / taw-theme v1.11.9: new taw_critical_css_entry filter on ViteLoader's critical CSS inlining, so a full-page CPT template with its own above-the-fold hero can swap in a different critical file per request." tags={["taw/core", "taw-theme", "feature"]}>
...
<Update label="2026-09-14" description="taw/core v1.33.0 / taw-theme v1.11.8: on_submit callbacks now receive the new submission's post ID as a second, optional argument; SubmissionsHandler::saveSubmission() also records a _taw_user_agent meta key." tags={["taw/core", "taw-theme", "feature"]}>
...
<Update label="2026-09-14" description="taw/core v1.32.0: new Catechism Reader Corpus — edition-parameterized part→section→chapter→question/answer reader mirroring the Bible corpus's SQLite/MySQL dual-backend architecture, applying every lesson from that corpus's production incidents from day one." tags={["taw/core", "feature"]}>

## September 2026 — Catechism Reader Corpus, edition-parameterized from day one

A generic `TAW\Core\Corpus\Catechism\*` namespace mirroring [Bible Reader Corpus](/taw-core#bible-reader-corpus)'s architecture one hierarchy level deeper — part → section → chapter → numbered question/answer, matching a catechism's own structure. Landed via [`Relmaur/taw-core#28`](https://github.com/Relmaur/taw-core/pull/28), tagged **v1.32.0**.
...
<Update label="2026-09-11" description="taw/core v1.31.1 / taw-theme v1.11.5: fix — MySQL FULLTEXT boolean-mode search silently dropped words shorter than innodb_ft_min_token_size (default 3), so any Spanish query containing de/la/el/en/un/... returned zero results. Confirmed against a real MySQL server." tags={["taw/core", "taw-theme", "bugfix"]}>

## September 2026 — MySQL FULLTEXT search ignored short/unindexed words

Verified against a real MySQL server (fsspx-taw theme session): `books()`/`chapter()`/full corpus import all confirmed byte-for-byte correct, but `+amor +de +Dios` returned zero results even though three verses genuinely contain all three words — `+amor +Dios` (dropping "de") returned the correct three. Landed via [`Relmaur/taw-core#25`](https://github.com/Relmaur/taw-core/pull/25) (closes `#24`), tagged **v1.31.1**, with a paired `taw-theme` release tagged **v1.11.5**.
...
<Update label="2026-09-11" description="taw/core v1.31.0 / taw-theme v1.11.4: MySQL-backed fallback for the Bible Reader Corpus when pdo_sqlite isn't available (confirmed absent on real WPMUdev managed hosting) — corpus:export produces a portable JSON export, corpus:install now accepts either format, BibleEndpoint resolves the right backend transparently." tags={["taw/core", "taw-theme", "feature"]}>

## September 2026 — MySQL-backed fallback for the Bible Reader Corpus

`bin/taw corpus:install` against the real ~18MB Straubinger corpus file on a live client site (WPMUdev managed hosting) fataled with `PDOException: could not find driver` — `pdo_sqlite`/`sqlite3` are missing from both PHP-FPM and CLI on that host, on two PHP versions, and WPMUdev declined to add them. `$wpdb` (`mysqli`) is guaranteed on every WordPress host since WP core itself can't function without it; `pdo_sqlite` is not guaranteed anywhere. Landed via [`Relmaur/taw-core#23`](https://github.com/Relmaur/taw-core/pull/23) (closes `#22`), tagged **v1.31.0**, with a paired `taw-theme` release tagged **v1.11.4**.

- **`ProtectedSqlite::isAvailable()`** — real capability detection: `extension_loaded('pdo_sqlite')` *and* a real `sqlite::memory:` connection attempt, since a loaded extension isn't always functional on every host build.
- **`TAW\Core\Corpus\Bible\BibleReaderInterface`** extracted (deferred in the original design until a second implementation existed to shape it against — that point has arrived); `BibleReader` implements it with zero internal changes, so every host where `pdo_sqlite` already works sees no behavior change at all.
- **`MysqlBibleReader`** — an independent, MySQL-backed mirror of the same read contract: `books()`/`chapter()` via `$wpdb` joins, `searchVerses()`/`searchNotes()` via MySQL boolean-mode `FULLTEXT` (`+word1 +word2`, translating the same AND-of-words semantics the SQLite search fix already established) with hand-built `<mark>` excerpts (MySQL has no `snippet()`).
- **`bin/taw corpus:export <sqlite-path> <json-path>`** — dumps a corpus to portable JSON on a machine that has `pdo_sqlite`, for installing on a target host that doesn't.
- **`bin/taw corpus:install`** — now content-sniffs `.sqlite` vs. portable JSON and routes to the matching backend; a `.sqlite` source on a host without `pdo_sqlite` now fails with a clear message pointing at `corpus:export`, instead of a bare `PDOException`.
- **`BibleEndpoint`** resolves SQLite first (unconditionally, whenever available and installed), MySQL otherwise.

See [Bible Reader Corpus](/taw-core#bible-reader-corpus) for the full reference, and `docs/adr/0002-corpus-mysql-fallback.md` in `taw-core` for the architectural reasoning. `Rag\Storage`'s vector chunks remain `pdo_sqlite`-only — a MySQL-backed vector store is a plausible follow-up, flagged as separate, harder scope (continuous writes from ongoing post saves don't fit the export-once-to-JSON approach this fix relies on).

</Update>

<Update label="2026-09-11" description="taw/core v1.30.0 / taw-theme v1.11.3: the RAG chatbot is now opt-in (RagSettings::enable()), matching every other optional subsystem — settings page, knowledge bases, WP-content ingestion, and POST taw/v1/chat all stay off by default." tags={["taw/core", "taw-theme", "feature"]}>

## September 2026 — RAG chatbot becomes opt-in

`Theme::boot()` unconditionally wired the entire Sovereign Hybrid-RAG Chatbot subsystem — every TAW site got the Settings → TAW Chatbot page, WP-content ingestion, and the public `POST taw/v1/chat` endpoint whether or not the developer wanted it. `Lucide`, `MediaFolders`, and the [Bible Reader Corpus](/taw-core#bible-reader-corpus)'s `BibleEndpoint` all follow an explicit `enable()` opt-in flag; the chatbot — arguably the subsystem with the highest bar to actually need (an LLM endpoint + `TAW_RAG_API_KEY`) — was the one exception. Landed via [`Relmaur/taw-core#20`](https://github.com/Relmaur/taw-core/pull/20) (closes `#19`), tagged **v1.30.0**, with a paired `taw-theme` release (`Relmaur/taw-theme` main, tagged **v1.11.3**).

- **`RagSettings::enable()`/`isEnabled()`** — same shape as `Lucide::enable()`/`MediaFolders::enable()`/`BibleEndpoint::enable()`. Call it in `inc/customizations.php` before `Theme::boot()`.
- **`taw-theme`'s `Blocks/Chatbot` widget** now checks `RagSettings::isEnabled()` in both `header.php` (asset enqueue) and `footer.php` (render) — previously it always rendered and would have `POST`ed to a REST route that no longer registers on a site that hasn't opted in.
- **`inc/customizations.php`** gets a commented-out `RagSettings::enable();` line, matching the Lucide/`icon` field type's posture (off by default, one line to turn on).

</Update>

<Update label="2026-09-11" description="taw/core v1.29.3 / taw-theme v1.11.2: two bugs found running bin/taw corpus:install on a live client site — ProtectedSqlite/Corpus\Storage silently killed pre-boot CLI usage, and corpus:install was never registered in taw-theme's bin/taw. Also fixed: Bible search required an exact phrase match." tags={["taw/core", "taw-theme", "bugfix"]}>

## September 2026 — corpus:install pre-boot fix, bin/taw registration, and Bible search matching

Two bugs found running `bin/taw corpus:install` against the real ~18MB Straubinger corpus file on a live client site (fsspx-taw), plus one more found while fixing them. Landed via [`Relmaur/taw-core#18`](https://github.com/Relmaur/taw-core/pull/18) (closes `#17`), tagged **v1.29.3**; `bin/taw` registration landed directly on `taw-theme` main, tagged **v1.11.2**.

- **`ProtectedSqlite`/`Corpus\Storage` silently killed pre-boot CLI usage.** Both carried `if (!defined('ABSPATH')) exit;`, but `CorpusInstallCommand::execute()` calls `ProtectedSqlite::looksLikeSqliteFile()` before `require $wpLoad` — so the guard fired and exited the whole process with zero output. The exact pitfall this repo's own `CLAUDE.md` already documents for `Content\*` (v1.25.1). Fixed by dropping the guard from both, matching `Content\*`'s precedent — neither class has a WordPress dependency of its own. A subprocess regression test (`Storage\PreBootAutoloadTest`, mirroring `Content\PreBootAutoloadTest`) now runs with no `ABSPATH` defined at all, the one test shape that can actually catch this.
- **`corpus:install` was never wired into `bin/taw`.** The command class shipped in `taw-core` v1.29.0's `src/CLI/`, but canonical `taw-theme`'s `bin/taw` never got the matching registration (compare `content:reindex-kb`, which got both a class and a registration when it shipped) — the command was unreachable from any fresh checkout.
- **Bible search required an exact contiguous phrase.** `BibleReader::escapeFtsPhrase()` wrapped the whole query as one FTS5 literal phrase, so `searchVerses('Dios amor')` silently returned nothing unless those words were adjacent in that exact order — which fails for almost any realistic multi-word search. Every word is now quoted independently and ANDed together.

</Update>

<Update label="2026-09-11" description="taw/core v1.29.0: Bible Reader Corpus — a framework-level, developer-installed reference-corpus subsystem (storage, CLI install, a Straubinger-schema Bible reader, and a rate-limited REST surface), built for the fsspx-taw client site's Bible reader." tags={["taw/core", "feature"]}>

## September 2026 — Bible Reader Corpus

A read-only REST surface over a developer-installed reference corpus — currently a Straubinger-translation Spanish Catholic Bible, built for the fsspx-taw client site's Bible reader (theme-side page/Block shipping separately) but framework-level and reusable at the storage layer. Landed via [`Relmaur/taw-core#16`](https://github.com/Relmaur/taw-core/pull/16) (closes `#15`), tagged **v1.29.0**.

- **`TAW\Core\Storage\ProtectedSqlite`** — the RAG chatbot's "protected uploads directory + exception-mode PDO open + magic-byte SQLite validation" plumbing, extracted and shared rather than duplicated for this new subsystem.
- **`TAW\Core\Corpus\Storage`** — `wp-content/uploads/taw-private/corpus/`, deliberately separate from the RAG chatbot's `taw-private/rag/`: a reference corpus meant for direct reading and a RAG knowledge base meant for chatbot embedding are different concerns that happen to share storage mechanics.
- **`php bin/taw corpus:install <path> <filename>`** — CLI-only, not a wp-admin upload form. A curated dataset a developer places once per environment, not end-user content.
- **`TAW\Core\Corpus\Bible\BibleReader`** — books grouped by testament/division, per-chapter verses + overlapping section headings + overlapping notes, and FTS5 search over verses or notes. Not `final` — subclassable so a theme can point at a different installed filename or override read behavior.
- **`GET /taw/v1/bible/books`, `.../books/{slug}/chapters/{n}`, `.../search`** — opt-in (`BibleEndpoint::enable()`), public (no auth — this is public Scripture text), rate-limited regardless. The endpoint resolves its reader through `apply_filters('taw_corpus_bible_reader', ...)`.

See [Bible Reader Corpus](/taw-core#bible-reader-corpus) for the full reference, and `docs/adr/0001-reference-corpus-storage.md` in `taw-core` for the architectural reasoning behind each of these decisions.

</Update>

<Update label="2026-09-11" description="taw/core v1.28.1 / taw-theme v1.11.1: fix — a post that lost RAG eligibility (unpublished, trashed, password-protected) after being indexed stayed searchable via the chatbot indefinitely. It's now de-indexed immediately." tags={["taw/core", "taw-theme", "bugfix"]}>

## September 2026 — RAG chatbot: de-index posts that become ineligible

`PostIndexer::onSavePost()` only ever handled the "should this post be indexed" path — a post that lost eligibility *after* being indexed (unpublished, moved to draft/private, trashed, or password-protected) stayed searchable through `search_knowledge_base` indefinitely, since the method just returned without touching its existing vectors. A real privacy gap, not just a staleness one. Landed via [`Relmaur/taw-core#13`](https://github.com/Relmaur/taw-core/pull/13), tagged **v1.28.1**, with a matching `taw-theme` v1.11.1 (`composer.lock` only — no theme code changed).

- Eligibility now also excludes password-protected posts outright (`post_password === ''`), not just publish status + indexed post type.
- Any post that isn't (or is no longer) eligible has its vectors removed **immediately, inline** — unlike the embed-on-publish path, a delete has no external API call to avoid blocking on, so there's no reason to defer it to WP-Cron.

</Update>

<Update label="2026-09-11" description="taw/core v1.28.0: Sovereign Hybrid-RAG Chatbot — content-agnostic knowledge bases. Upload any .sqlite file in wp-admin; a single search_knowledge_base tool answers from it, the site's own content, or both." tags={["taw/core", "taw-theme", "feature"]}>

## September 2026 — Sovereign Hybrid-RAG Chatbot: content-agnostic knowledge bases

A visitor-facing chat widget that answers from any number of named **knowledge bases** — the site's own WordPress content, plus any `.sqlite` file an admin uploads from wp-admin — with an OpenAI-compatible LLM doing semantic search across whichever one a question calls for. Landed via [`Relmaur/taw-core#12`](https://github.com/Relmaur/taw-core/pull/12) (closes `#11`), tagged **v1.28.0**, with a paired `taw-theme` release ([`Relmaur/taw-theme#3`](https://github.com/Relmaur/taw-theme/pull/3)).

<Callout kind="info">
  This supersedes **v1.27.0** ([`#10`](https://github.com/Relmaur/taw-core/pull/10), tagged the same day), which shipped the same chat widget and REST endpoint but with the reference-data layer hardcoded to two specific schemas (`lookup_bible`/`lookup_catechism`). On review that was the wrong shape — v1.27.0 was superseded before any real consumer integrated against it. If you're on v1.27.0, upgrade straight to v1.28.0; there's no migration path from the old schema-specific tools, only a clean replacement.
</Callout>

- **Content-agnostic by design.** No particular schema is assumed or required of an uploaded `.sqlite` file — every table is scanned, every column with SQLite TEXT affinity is extracted, chunked, embedded, and made searchable. A table with no text column is skipped.
- **`Settings → TAW Chatbot → Knowledge Bases`** — upload a `.sqlite` file with a name and description; that's the entire setup. Ingestion runs via WP-Cron, never on the upload request. The upload handler validates the SQLite magic-byte header before accepting a file.
- **The site's own content is just another knowledge base** — a built-in, non-deletable `wp-content` entry, unified into the same registry and the same search tool as uploaded ones, rather than a separate dedicated tool.
- **One tool, not three.** `search_knowledge_base(knowledge_base, query)` replaces the old `lookup_bible`/`lookup_catechism`/`search_unstructured_archive` trio. Its `knowledge_base` enum is built fresh from the registry on every request, so a newly-uploaded knowledge base is searchable the moment ingestion finishes.
- **`Settings → TAW Chatbot`** — LLM base URL/model, indexed post types for the site's own content, chunk size/overlap, max tool-call iterations, and whether anonymous visitors can chat. The LLM API key is wp-config-constant-only (`TAW_RAG_API_KEY`), never an options-table field.
- **`POST /wp-json/taw/v1/chat`** — public by default, defended by unconditional rate limiting (20 requests/10 min per IP) rather than a login requirement.
- **`Blocks/Chatbot`** (taw-theme) — a site-wide floating widget, unchanged by this redesign since it only depends on the `/taw/v1/chat` request/response shape. Markdown replies render via `marked`, sanitized with `DOMPurify`.
...
<Update label="2026-09-10 (pending release)" description="taw/core v1.26.0: Content Interchange becomes a whole-site state migration path — post authorship, opt-in users / comments / environment settings, a --migrate shorthand, Metabox-backed CPT auto-export, and a slug-less-draft fix." tags={["taw/core", "taw-theme", "feature"]}>

## September 2026 — Content Interchange → whole-site state migration

[Content Interchange](/content-interchange) now moves a site's **state**, not just its content. All new behaviour is opt-in and backward compatible; the importer accepts schema `1.0` and `1.1`. Landed via [`Relmaur/taw-core#8`](https://github.com/Relmaur/taw-core/pull/8) (closes `#7`), tagged **v1.26.0**, with a paired `taw-theme` release.
...
- **Post authorship.** Every exported post now carries `author` (`{login, email}` — never a numeric ID) plus `comment_status` / `ping_status`. On import the author resolves by login, then email, then falls back to the importing user with a warning.
- **`--with-users`** exports a `users[]` section (`login`, `email`, `display_name`, `roles[]`, profile meta, `user_registered`). Password hashes only with the separate `--with-user-passwords`. Import matches by login → email, creates if absent, and **sanitises roles against the roles the target site actually defines** — a role the target lacks is never granted.
- **`--with-comments`** exports comments on the exported posts with `parent_ref` threading; import remaps them to their post by slug, rebuilds `comment_parent`, and recomputes counts.
- **`--with-settings`** exports a *second*, opt-in option allowlist for environment settings — `permalink_structure`, `timezone_string`, `sticky_posts` (as slugs), `date_format`, and more — plus a `taw_content_export_settings_options` filter. These are **import-gated too**: `content:import` skips them unless you also pass `--with-settings`.
- **`content:export --migrate`** = `--with-users --with-settings --all-media --include-drafts` (not passwords or comments — those stay explicit). The automatic rollback snapshot is now maximal-scope, so undoing a `--migrate` import is complete.
- **Slug-less drafts no longer duplicate on re-import.** `draft` / `pending` / `auto-draft` posts are excluded by default; `--include-drafts` re-adds them and matches slug-less posts on a composite `(type, sha1(title|date_gmt))` key.
- **Metabox-backed CPTs auto-export.** Any post type with a `TAW\Core\Metabox\Metabox` attached is now included even when `public => false`. Sites that added a manual `taw_content_export_post_types` filter just for a content CPT (for example FSSPX's `activity` Mass schedule) can drop it.
- **`--all-media`** also exports attachments not referenced by any post/field; media records now carry `title` / `description`, set on the sideloaded attachment.
- **Import ordering** is deterministic and dependency-first: users → terms → media → posts → comments → settings.
- **`content:diff`** diffs the new `users` and `comments` sections.
- The `Tools → TAW Data` export screen gains checkboxes for the new scopes (passwords / comments behind an "Advanced" disclosure). `GET /wp-json/taw/v1/content/export` stays **content-only** — no user or settings export over REST.
- **Downstream:** the `taw-theme` scaffold `bin/taw` already registers `content:*`. Any site keeping an `owner: site` content-sync skill should update its `pull` / `push` sections for the new `--with-*` / `--migrate` flags.

</Update>

<Update label="2026-09-10 (pending release)" description="taw/core v1.25.1: content-interchange round-trip fixes — content:import (and content:diff) no longer print nothing, and export → import of the same site is now a verified no-op." tags={["taw/core", "taw-theme", "bugfix"]}>

## September 2026 — Content interchange round-trip fixes

Three bugs in the v1.25.0 [Content Interchange](/taw-core#content-interchange), all reproduced live against a populated site and regression-tested. Landed via [`Relmaur/taw-core#6`](https://github.com/Relmaur/taw-core/pull/6) (closes `#5`), tagged **v1.25.1**.

- **`content:import` printed nothing** (exit 0, no plan table, no "no changes" line). `ContentImportCommand` references a `TAW\Core\Content\*` class before `require wp-load.php`; that autoloaded the class file, whose `if (!defined('ABSPATH')) exit;` guard fired (WordPress not booted yet) and silently killed the process. The guard is removed from the six `Content\*` classes the `content:*` commands load before boot — they are pure class definitions, like `TAW\Helpers\Framework` / `TAW\CLI\WpLoader`, which already omit it. This also fixes **`content:diff`**, which was silent for the same reason.
- **`page_on_front` / `page_for_posts` broke the round-trip.** The exporter renders them as page slugs (portable); the importer compared the slug against the stored integer ID → a false "changed", and an apply would have written the slug into `page_on_front`, breaking the static front page. The importer now resolves these back to the local post ID before both the diff and the write. `post.parent` (slug) and `featured_media` (filename) were audited — already resolved on write; `featured_media` is now also diffed by filename.
- **Empty / default values showed as changes.** A field stored as `""` (or absent, or at its default) compared unequal to the same empty value in the snapshot. Both sides now normalize through `FieldCodec::decode` plus a canonical key that collapses `""` / `null` / `[]` / `false` / missing to one token, and aligns `"1"` ↔ `true`, `"[…]"` ↔ the decoded array, `"42"` ↔ 42.
...
<Update label="2026-09-10 (pending release)" description="taw/core v1.25.0: portable content interchange — a Content Exporter/Importer, a Tools → TAW Data screen, bin/taw content:export | content:import | content:diff, and REST-registered field meta for headless integrations." tags={["taw/core", "taw-theme", "feature"]}>
...
**taw-theme** picks up `taw/core` v1.25.0 and registers the three `content:*` commands in `bin/taw` (v1.9.0). Landed via [`Relmaur/taw-core#4`](https://github.com/Relmaur/taw-core/pull/4) (closes `#3`).

See [Content Interchange](/taw-core#content-interchange).

</Update>

<Update label="2026-09-09 (pending release)" description="taw/core v1.24.0: user-enumeration hardening — the public /wp/v2/users REST routes are now hidden from anonymous requests by default, closing the ?rest_route=/wp/v2/users leak that host WAFs miss. The taw-theme scaffold gains a site-owned inc/security.php." tags={["taw/core", "taw-theme", "security"]}>
...
- Only **anonymous** requests are affected. Any logged-in user keeps the full API, so the block editor's `/wp/v2/users?who=authors` author selector still works for Editors (who lack `list_users`).
- `/wp/v2/users/me` always stays available — the mobile apps, Jetpack, and the block editor need the authenticated self-lookup, and it already returns `401` without a login.
- Escape hatch for headless / integration sites: `add_filter('taw_security_hide_users_endpoint', '__return_false')`.
...
See [Security / Hardening](/taw-core#security--hardening).

</Update>

<Update label="2026-09-03 (pending release)" description="taw-hub-companion 0.2.0: the plugin now self-updates from its GitHub releases — 'Update available' on every site, plus opt-out background auto-updates, so fleet-wide companion updates need no SSH or per-site zip upload" tags={["taw-hub-companion", "feature"]}>
...
- Every site shows the standard **"Update available"** row; `wp plugin update taw-hub-companion` works.
- Unless `TAW_HUB_COMPANION_AUTO_UPDATE` is `false` in `wp-config.php`, the plugin opts itself into WordPress's **background auto-update cron** — a tagged release reaches the whole fleet within a cron cycle (~12 h), no clicking.
...
See [TAW Hub Companion](/taw-hub-companion).

</Update>

<Update label="2026-09-03 (pending release)" description="taw/core v1.23.0: new php bin/taw hub:enroll command — one-shot site enrolment into a TAW Hub fleet, with cryptographic verification of the Hub's signed reply" tags={["taw/core", "feature"]}>

## September 2026 — One-command fleet enrolment

Joining a site to a [TAW Hub](https://github.com/Relmaur/taw-hub) fleet used to end with a manual step: read the site's public key and key id, then register them on the Hub by hand. `php bin/taw hub:enroll` closes that gap.

It reads the identity the `taw-hub-companion` plugin generated on activation (`taw_hub_companion_public_key` / `_key_id`), the Hub URL (`TAW_HUB_URL`), and a **one-time enrolment token** minted on the Hub (`--token`, or `TAW_HUB_ENROLMENT_TOKEN`), and `POST`s them to the Hub's `POST /api/fleet/enroll` ([taw-hub ADR-0011](https://github.com/Relmaur/taw-hub/blob/main/docs/ADR/0011-site-enrolment.md)).
...
<Update label="2026-09-03 (pending release)" description="taw-hub-companion 0.1.7: two new signed routes — GET /vulnerabilities (surfaces the site's own scanner findings from Defender or Wordfence, no paid feed) and GET /inventory/checksums (per-component file manifest for integrity + backdoor-on-update diffing)" tags={["taw-hub-companion", "feature"]}>
...
- **Defender / Defender Pro** — reads the `defender_scan` / `defender_scan_item` tables: `vulnerability` items (one finding per bug, with CVSS), plugins closed on wp.org (→ `removed`), and abandoned plugins (→ `abandoned`). Verified against Defender 6.2.4. Checked first — it's the fleet standard.
- **Wordfence** — reads the `wfIssues` table for known vulnerabilities, abandoned plugins, and directory removals; reports `last_scan_at` so the Hub can spot a stale scanner. Verified against Wordfence 8.2.x.
...
Summary mode (no `slug`) returns just `tree_hash` + `file_count` per active component — a cheap "did anything change" poll; detail mode (`?slug=`) adds the full `relpath → sha256` map. Only executable / script file types are hashed; `node_modules` and `.git` are skipped; symlinks are not followed. `tree_hash` has a pinned canonical form (byte-sorted `"<relpath>:<sha256>\n"` lines, sha256'd) so the Hub can recompute it from a wordpress.org release zip and compare.
...
<Update label="2026-09-02 (pending release)" description="taw-hub-companion 0.1.3: new signed GET /inventory route — a security-focused SBOM the Hub polls for fleet vulnerability correlation and abandoned-plugin detection" tags={["taw-hub-companion", "feature"]}>

## September 2026 — Fleet inventory: `/inventory` on the companion

The first slice of a fleet-security initiative: giving the [TAW Hub](https://github.com/Relmaur/taw-hub) the visibility to defend a fleet against the WordPress plugin threats that AI-accelerated exploitation has made sharper — known-vuln plugins, quiet supply-chain backdoors, and abandoned plugins left installed for years.
...
<Update label="2026-09-02 (pending release)" description="taw/core: MetaboxOrder::lockFromTemplate() now resolves page-{slug}.php and home.php templates, not just explicitly-selected ones and front-page.php" tags={["taw-core", "fix"]}>
...
Backward-compatible — pages that already ordered correctly are unaffected. See [Locking Metabox Order](/taw-core#locking-metabox-order).

</Update>

<Update label="2026-09-01" description="taw-hub-companion 0.1.1–0.1.2: first live fleet enrolments (dev + production), TawRunner hardened for php-fpm and proc_open-disabled managed hosts" tags={["taw-hub-companion", "fix"]}>
...
- **0.1.1** — resolve a real CLI `php`. `PHP_BINARY` is `php-fpm` under the SAPI that serves the Hub's requests, so `bin/taw` never actually ran; now it resolves a CLI interpreter beside the SAPI binary / on `PATH`, override with `taw_hub_companion_php_binary`.
- **0.1.2** — degrade cleanly where `proc_open` is disabled (most managed hosts). `/framework/sync` and `/taw` return `503 exec_unavailable`; `/health` reports `"exec_available": false`; `/health` and `/logs` keep working (they never shell out). Error responses no longer leak stack traces or server paths.

See [TAW Hub Companion](/taw-hub-companion).

</Update>

<Update label="2026-09-01 (pending release)" description="taw/core: a new structured Logger replacing scattered error_log() calls, plus bin/taw log:tail; taw-hub-companion gains a signed /logs route" tags={["taw-core", "taw-hub-companion", "feature"]}>
...
See [Logging](/taw-core#logging) and [TAW Hub Companion → Routes](/taw-hub-companion#routes).

</Update>

<Update label="2026-08-27 (pending release)" description="taw/core: update-theme now preserves site-authored Claude/agent skills across a framework sync instead of deleting them" tags={["taw-core", "taw-theme", "feature"]}>
...
The `skills-dir` manifest type and the `owner:` marker are declared in `taw/core`'s `resources/update-manifest.json` under `skillsReconcile`; `TAW\CLI\SyncCommand` implements the reconcile. See [AI Integration](/ai-integration#claude-code-skills).

</Update>

<Update label="2026-08-23 (pending release)" description="taw/core: Form gains image/wysiwyg fields + an on_submit hook; new PagePassword API; taw-theme ships a password-gated client post-submission portal" tags={["taw-core", "taw-theme", "feature", "security"]}>
...
New `TAW\Core\Auth\PagePassword` gates a page template behind a single shared password, declared directly in the template before any output — not a wp-admin toggle. Fails closed (denies access) rather than gracefully degrading when misconfigured, since this is access control, not a supplementary check. See [Page Password Protection](/taw-core#page-password-protection) and [On Submit Callback](/taw-core#on-submit-callback).
